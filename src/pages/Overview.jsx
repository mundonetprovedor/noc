    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 300px', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {/* Operational Banner */}
          <div className="glass-card" style={{ padding: '1rem', border: stats.criticalTriggers === 0 ? '1px solid var(--success)' : '1px solid var(--danger)', display: 'flex', alignItems: 'center', gap: '0.5rem', background: stats.criticalTriggers === 0 ? 'rgba(0, 255, 157, 0.05)' : 'rgba(255, 71, 87, 0.05)' }}>
            <CheckCircle size={18} color={stats.criticalTriggers === 0 ? 'var(--success)' : 'var(--danger)'} />
            <span style={{ fontSize: '0.9rem', fontWeight: 700, color: stats.criticalTriggers === 0 ? 'var(--success)' : 'var(--danger)', letterSpacing: '1px' }}>
              REDE {stats.criticalTriggers === 0 ? '100% OPERACIONAL' : 'COM INCIDENTES CRÍTICOS'}
            </span>
            <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: 'var(--text-secondary)' }}>SW CORE - TEMPO REAL</span>
          </div>

          <section className="glass-card" style={{ padding: '0', minHeight: '550px', background: 'var(--surface-dark)', border: '1px solid var(--glass-border)', overflow: 'hidden' }}>
            <div style={{ padding: '1.2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.02)' }}>
              <h3 style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '1px', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <Activity size={18} color="var(--accent-blue)" /> MONITORAMENTO TÉCNICO (FEED ZABBIX)
              </h3>
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                <button
                  onClick={() => setIsAlarmEnabled(!isAlarmEnabled)}
                  style={{
                    background: isAlarmEnabled ? 'rgba(0, 210, 255, 0.1)' : 'rgba(255, 77, 77, 0.1)',
                    border: '1px solid var(--glass-border)',
                    borderRadius: '8px',
                    padding: '0.4rem 0.8rem',
                    color: isAlarmEnabled ? 'var(--accent-blue)' : 'var(--danger)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '0.4rem',
                    fontSize: '0.7rem',
                ...
                >
                  {isAlarmEnabled ? <Volume2 size={14} /> : <VolumeX size={14} />}
                  ALERTA SONORO: {isAlarmEnabled ? 'ATIVO' : 'MUDO'}
                </button>
                ...
                <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700 }}>{incidentHistory.filter(i => i.status === 'active').length} ATIVOS</span>
                </div>
                <div style={{ width: '1px', height: '12px', background: 'var(--glass-border)' }} />
            ...
            <div className="incident-list" style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 100px 120px', padding: '0.8rem 1.5rem', background: 'var(--surface-low)', borderBottom: '1px solid var(--glass-border)', fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                <div>Horário</div>
                <div>Host / Incidente Detectado</div>
                <div>Duração</div>
                <div style={{ textAlign: 'right' }}>Severidade</div>
              </div>

              <AnimatePresence mode="popLayout">
                {filteredIncidentHistory.length === 0 ? (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    style={{ textAlign: 'center', padding: '6rem', color: 'var(--text-secondary)' }}
                  >
                    <CheckCircle size={48} color="var(--success)" style={{ opacity: 0.1, marginBottom: '1.2rem' }} />
                    <p style={{ fontWeight: 600, fontSize: '0.9rem' }}>Nenhum incidente {activeCategory !== 'TODOS' ? `em ${activeCategory}` : ''} no momento.</p>
                  </motion.div>
                ) : (
                  filteredIncidentHistory.slice(0, 15).map(incident => {
                    const sev = getPriorityDetails(incident.priority);
                    return (
                      <motion.div
                        key={incident.triggerid}
                        layout
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        style={{
                          ...
                          alignItems: 'center',
                          background: incident.status === 'active' ? sev.bg : 'rgba(0, 255, 136, 0.02)',
                          borderLeft: `3px solid ${incident.status === 'active' ? sev.color : 'var(--success)'}`,
                        }}
                      >
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
                          {new Date(incident.lastchange * 1000).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.1rem' }}>
                          <span style={{ fontSize: '0.65rem', color: sev.color, fontWeight: 900, textTransform: 'uppercase' }}>
                            {incident.hosts?.[0]?.name}
                          </span>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', fontWeight: 700 }}>
                          {formatDuration(incident.lastchange)}
                        </div>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <span style={{
                            ...
                            borderRadius: '4px',
                            background: incident.status === 'active' ? sev.color : 'var(--success)',
                            color: '#fff',
                            minWidth: '90px',
                            display: 'inline-block',
                            textAlign: 'center'
                          }}>
                            {incident.status === 'active' ? sev.name.toUpperCase() : 'RESOLVIDO'}
                          </span>
                        </div>
                      </motion.div>
                    );
